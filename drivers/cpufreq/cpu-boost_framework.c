...
{
	suspended = true;
	pr_info("%s: CPU-boost framework suspended!\n", __func__);
}

void cpu_boost_startup(void)
{
	suspended = false;
	pr_info("%s: CPU-boost framework resumed!\n", __func__);
}

static struct kobj_attribute cpu_boost_enabled = __ATTR(enabled, 0644, cpu_boost_enabled_status_read, cpu_boost_enabled_status_write);
...
static int __init cpu_boost_init(void)
{
	...
	if (cpu_boost_kobject) ret = sysfs_create_group(cpu_boost_kobject, &cpu_boost_attr_group);
	if (ret) kobject_put(cpu_boost_kobject);
